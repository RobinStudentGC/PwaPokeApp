...
function onPageInput(event) {
    ...
    if (!isNaN(value)) emit('change', value);
}
</script>

<template>
    <div class="pagination">
        <button class="mdc-button mdc-button--raised" :disabled="props.currentPage <= 1" @click="emit('previous')">
        ...
        <div class="pagination__label">
            ...
            <input type="number" class="pagination__input" min="1" :max="props.pageCount" :value="props.currentPage"
                @change="onPageInput" />
            <span class="pagination__text">of {{ props.pageCount }}</span>
        </div>

        <button class="mdc-button mdc-button--raised" :disabled="props.currentPage >= props.pageCount"
            @click="emit('next')">
            <span class="mdc-button__ripple"></span>
